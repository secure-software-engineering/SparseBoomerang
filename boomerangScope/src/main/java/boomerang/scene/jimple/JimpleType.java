...
public class JimpleType implements Type {
  ...
  public boolean isSupertypeOf(String subType) {
    ...
    /*if (thisClass.equals(subClass)) {
      return true;
    }

    if (thisClass.isInterface()) {
      if (subClass.isInterface()) {
        return Scene.v().getActiveHierarchy().getSuperinterfacesOfIncluding(subClass).contains(thisClass);
      } else {
        return Scene.v().getActiveHierarchy().getSubinterfacesOfIncluding(thisClass).contains(thisClass);
      }
    } else {
      return Scene.v()
          .getActiveHierarchy()
          .getSuperclassesOfIncluding(subClass)
          .contains(thisClass);
    }*/
  }
